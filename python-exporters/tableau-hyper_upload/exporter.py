...
class CustomExporter(Exporter):
    ...
    def open(self, schema):
        ...
        print("\tAuth information: {0}".format(s_info.build_number))

        server.auth.sign_in(tableau_auth)

        project_req_option = TSC.RequestOptions()
        project_req_option.filter.add(
            TSC.Filter(
        ...
        project_item, pagination_item = server.projects.get(req_options=project_req_option)
        print("\tItems found: {0}".format(project_item))
        if project_item:
        ...
        if len(datasource_match) > 0:
            ...
            print('WARN: Found existing table {} with id {}, will be overwritten'.format(
                self.datasource.name.encode('utf-8'), self.datasource.id))
