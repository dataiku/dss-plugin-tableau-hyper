...
from tableau_server_utils import (
    get_project_from_name,
    get_full_list_of_projects,
    get_tableau_server_connection,
)
from custom_exceptions import InvalidPluginParameter, TemporaryFileAlreadyDeleted
...
class TableauHyperExporter(Exporter):
    ...
    def open(self, schema):
        ...
        self.output_file = TemporaryFile(file_name=self.output_file_name + ".hyper")

        self.writer.schema_converter.set_dss_storage_types(schema)
        self.writer.create_schema(schema, self.output_file.get_file_path())
        logger.info("Defined the temporary output file: {}".format(self.output_file.get_file_path()))
        ...
        if self.output_file is not None:
            with self.server.auth.sign_in(self.tableau_auth):
                self.server.datasources.publish(
                    self.tableau_datasource,
                    self.output_file.get_file_path(),
                    "Overwrite",
                )

            try:
                self.output_file.clean()
            except Exception as err:
                if err is not TemporaryFileAlreadyDeleted:
